...
def wave_fft(data, time, window):
    fft = np.array([np.fft.fft(data[i*window:(i+1)*window])[:int(window/2)+1] for i in range(int(len(data)/window))])
    newtime = np.array([time[i*window] for i in range(int(len(data)/window))])
    duration = (pd.to_datetime(np.max(time))-pd.to_datetime(np.min(time))).total_seconds()
    sample_frequency = len(data)/duration
    frequency = np.array([i*sample_frequency/window for i in range(fft.shape[1])])
...
def wave_polar_and_angle(BBmatrix, background):
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            ...
            k = VT[sort_index[-1],:]
            angle[i,j] = np.arccos(np.sum(k*background[i]) / (la.norm(k)*la.norm(background[i])))*180/np.pi
